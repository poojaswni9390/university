...
const Registration = ({ navigation }) => {
  ...
  const onSubmit = async () => {
    try {
      ...
      const registerResponse = await axios.post("http://192.168.65.168:3000/register", registerData);
      ...
      const educationData = educationEntries.map((entry) => ({
        ...
        skills: JSON.stringify(skills.map((skill) => ({
          skillName: skill.skill_name,
          proficiencyLevel: skill.proficiency_level,
        }))),
        certificate_info: JSON.stringify(certifications.map((cert) => ({
      ...
      for (const edu of educationData) {
        await axios.post("http://192.168.65.168:3000/education", edu);
      }
      ...
      await axios.post("http://192.168.65.168:3000/project-experience", projectExperienceData);
